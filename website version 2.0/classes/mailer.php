...
class Mailer {
	...
	function __construct()
	{
		...
		$this->phpMailer->Password = 'EverSoftsource2016';
	...
	{
		$token = 'ZaKKAWJjuqyxulR05427cMqVsrqgv9ro9TN';
        $to = $phone;
        $sender    = 'EClient';
	...
	public function sendForgetmail($email)
	{
		$token = Token::generate();
		$user = new User();
		$user->create('temp_token',array(
			'email' => $email, 
			'token' => $token,
		...
		try{
			...
			$this->phpMailer->Body = "You request for a forget password. Use this link to recover password: http://localhost/eclient/00.final%20site/recovery.php?token=$token. This token is only valid for next 30 minute.";
			$this->phpMailer->send();
			$this->phpMailer->ClearAllRecipients();
				return true;
			}catch(Exception $e){
