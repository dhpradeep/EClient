...
<head>
    ...
    {
        $.post("ajax_data.php",{
            username_data: '<?php echo Session::get('username'); ?>'
            },function (data) {
                console.log(data);
            }).fail(function(){
                console.log("error");
            ...
            <div class="header-menu">
                ...
                <div class="col-sm-7">
                    ...
                    <div class="header-left">
                        ...
                        $count = new Data();
                        $query1 = "SELECT * FROM notice WHERE active=? AND to_whom=? ORDER BY ID DESC LIMIT 5";
                        $param1 = array('unread',Session::get('username'));
                        $count = $count->getmultipledata($query1,$param1);
                        $total = $count->count();

                        //for printing the results
                        $notices = new Data();
                        $query = "SELECT * FROM notice WHERE to_whom=? ORDER BY ID DESC LIMIT 5";
                        $params = array(Session::get('username'));
                        $notices = $notices->getmultipledata($query,$params);
                        $notices = $notices->_results;

                        ?>
                        <div class="dropdown for-message">
                            <button onclick="makeallread()" class="btn btn-secondary dropdown-toggle" type="button" id="message" data-toggle="dropdown" aria-haspopup="true"
                                aria-expanded="false">
                                <i class="ti-email"></i>
                                <span class="count bg-primary"><?php echo $total; ?></span>
                            </button>
                            <div class="dropdown-menu" aria-labelledby="message">
                                <?php foreach($notices as $notice): ?>
                                <?php if($notice->active == 'unread'): ?>
                                <a class="dropdown-item media unread" href="<?php echo $notice->link; ?>">
                                <?php else: ?>
                                <a class="dropdown-item media read" href="<?php echo $notice->link; ?>">
                                <?php endif ?>
                                    <span class="message media-body">
                                        <span class="name float-left"><?php echo $notice->to_whom; ?></span>
                                        <p><?php echo $notice->data ;?></p>
                                    </span>
                                </a>
                                <?php endforeach ?>
                            </div>
